...
type Emittable = (typeof PING_EMITTABLE_ACTIONS)[number]

interface PingSpec {
  readonly monitor: string
  readonly action: Emittable | undefined
...
function readAction(args: ParsedArgs): Read<Emittable | undefined> {
  ...
  const allowed: readonly string[] = PING_EMITTABLE_ACTIONS

  if (!allowed.includes(given.value)) {
    return {
      ok: false,
      problem: `--action=${given.value} is not a check-in this SDK will send. Use start, success or fail, or leave --action off for a plain heartbeat — the server reads anything else as a heartbeat and marks the monitor up.`,
    }
  }
